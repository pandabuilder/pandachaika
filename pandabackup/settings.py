...
if MAIN_LOGGER and not os.path.exists(os.path.dirname(MAIN_LOGGER)):
...
LOGGING: dict[str, Any] = {
    ...
    "formatters": {
        "verbose": {"format": "%(levelname)s %(asctime)-.19s %(module)s " "%(process)d %(thread)d %(message)s[0m"},
        "simple": {"format": "%(levelname)s %(message)s[0m"},
        "console": {"format": "%(asctime)-.19s %(levelname)s [%(name)s] %(message)s"},
        "standard": {
...
STATICFILES_DIRS = (os.path.join(BASE_DIR, "viewer/assets"),)
...
if crawler_settings.STATIC_ROOT:
    STATIC_ROOT: str = crawler_settings.STATIC_ROOT
else:
...
if crawler_settings.mail_logging.enable:
    ...
    ADMINS = [
        ("Admin", crawler_settings.mail_logging.to),
    ]
    EMAIL_USE_TLS = True
