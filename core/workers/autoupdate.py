    ...
    def job(self) -> None:
        while not self.stop.is_set():
            ...
            if self.settings.providers[self.provider_name].autoupdater_enable:
                ...
                start_date = django_tz.now() - timedelta(seconds=int(self.timer)) - timedelta(days=self.settings.providers[self.provider_name].autoupdater_buffer_back)
                end_date = django_tz.now() - timedelta(days=self.settings.providers[self.provider_name].autoupdater_buffer_after)
