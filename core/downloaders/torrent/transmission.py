...
class Transmission(TorrentClient):
    ...
    def add_torrent(self, enc_torrent: Union[str, bytes], download_dir: Optional[str] = None) -> tuple[bool, Optional[str]]:
    ...
    def get_download_progress(self, download_list: list[tuple[str, TorrentClient.TorrentKey]]) -> list[tuple[TorrentClient.TorrentKey, float]]:
        ...
        torrents = self.trans.get_torrents(
            torrent_ids,
            timeout=25
        )
