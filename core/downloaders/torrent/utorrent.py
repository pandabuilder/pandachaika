...
class uTorrent(TorrentClient):
    ...
    def __init__(
        self, address: str = "localhost", port: int = 8080, user: str = "", password: str = "", secure: bool = True
    ) -> None:
    ...
    def add_torrent(
        ...
        try:
            response = requests.post(self.UTORRENT_URL, auth=self.auth, params=params, files=files, timeout=25).json()
            lf.close()
