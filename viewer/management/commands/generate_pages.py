...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-ps", "--public-stats", required=False, action="store", help=("Statically render the public stats page. ")
        )
    ...
    def generate_public_stats(self, output_html: str) -> None:
        ...
        stats_dict = {
            ...
            "top_10_tags": Tag.objects.filter(gallery__public=True)
            .distinct()
            .annotate(num_archive=Count("gallery"))
            .order_by("-num_archive")[:10],
            "top_10_artist_tags": Tag.objects.filter(scope="artist", gallery__public=True)
            .distinct()
            .annotate(num_archive=Count("gallery"))
            .order_by("-num_archive")[:10],
            "top_10_parody_tags": Tag.objects.filter(scope="parody", gallery__public=True)
            .distinct()
            .annotate(num_archive=Count("gallery"))
            .order_by("-num_archive")[:10],
        }
        ...
        for provider in providers:
            providers_dict[provider] = {
                ...
                "gallery": Gallery.objects.filter(public=True)
                .filter(filesize__gt=0, provider=provider)
                .aggregate(
                    Avg("filesize"),
                    Max("filesize"),
                    Min("filesize"),
