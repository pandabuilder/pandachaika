...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help=("Scan and register all new providers to the database. " "Already registered ones will be skipped."),
        )
        parser.add_argument(
            "-sc",
            "--scan",
            ...
            action="store_true",
            default=False,
            help=("Scan and show providers found on disk."),
        )
        parser.add_argument(
            "-r",
            "--register",
    ...
    def register_provider_from_constants(self, constants: ModuleType, provider_name: str) -> None:
        ...
        if not provider_instance:
            ...
            Provider.objects.create(
                slug=getattr(constants, "provider_name") if hasattr(constants, "provider_name") else provider_name,
                name=getattr(constants, "friendly_name") if hasattr(constants, "friendly_name") else provider_name,
