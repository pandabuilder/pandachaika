...
from viewer.forms import (
    ArchiveGroupSearchForm,
    ArchiveGroupCreateOrEditForm,
    ArchiveSearchForm,
...
def archive_group_details(request: HttpRequest, pk: Optional[int] = None, slug: Optional[str] = None) -> HttpResponse:
    ...
    if "extract_all" in get and request.user.has_perm("viewer.expand_archive"):
        archives_to_extract = archive_group_instance.archivegroupentry_set.select_for_update().filter(
            archive__extracted=False
        )
...
def archive_group_edit(request: HttpRequest, pk: Optional[int] = None, slug: Optional[str] = None) -> HttpResponse:
    ...
    if request.POST.get("submit-archive-group"):
        ...
        if edit_form.is_valid() and archive_group_entry_formset.is_valid():
            ...
            messages.success(request, message)
            logger.info("User {}: {}".format(request.user.username, message))
            event_log(request.user, "CHANGE_ARCHIVE_GROUP", content_object=new_archive_group, result="changed")
    ...
    if "add_to_group" in p:
        ...
        for archive in archives:
            if not ArchiveGroupEntry.objects.filter(archive=archive, archive_group=archive_group_instance).exists():
                ...
                messages.success(request, message)
                event_log(
                    request.user, "ADD_ARCHIVE_TO_GROUP", content_object=archive, reason=user_reason, result="added"
                )

        return HttpResponseRedirect(
            reverse("viewer:archive-group-edit", args=[archive_group_instance.title_slug])
            + "?"
    ...
    d.update(archive_group_entry_formset=archive_group_entry_formset, results=results_page)

    return render(request, "viewer/archive_group_edit.html", d)


@login_required
def archive_group_enter_reason(
    request: HttpRequest, pk: Optional[int] = None, slug: Optional[str] = None, tool: Optional[str] = None
) -> HttpResponse:
    try:
        if pk is not None:
            archive_group_instance = ArchiveGroup.objects.get(pk=pk)
    ...
    if request.method == "POST":
        ...
        if "confirm_tool" in p:
            ...
            if request.user.has_perm("viewer.delete_archivegroup") and tool == "delete":
                ...
                event_log(
                    request.user, "DELETE_ARCHIVE_GROUP", reason=user_reason, result="deleted", data=delete_report
                )
