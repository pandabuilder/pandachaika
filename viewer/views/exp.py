        ...
        for term in terms:
            ...
            if term.startswith("-"):
                tag_query = (
                    ...
                    if len(scope_name) > 1
                    else Q(tags__name__contains=scope_name[0])
                )
...
def api(
    request: HttpRequest, model: Optional[str] = None, obj_id: Optional[str] = None, action: Optional[str] = None
) -> HttpResponse:
        ...
        if model == "archive" and obj_id is not None:
            ...
            if action == "extract_toggle":
                try:
                    if actual_user and actual_user.has_perm("viewer.expand_archive"):
                        ...
                        return HttpResponse(
                            json.dumps({"result": "Admin only action."}), content_type="application/json; charset=utf-8"
                        )
                ...
                try:
                    if actual_user and actual_user.has_perm("viewer.expand_archive"):
                        ...
                        return HttpResponse(
                            json.dumps({"result": "error", "message": "You don't have permission for this action."}),
                            content_type="application/json; charset=utf-8",
                        )
                except Archive.DoesNotExist:
                    return HttpResponse(
                        json.dumps({"result": "error", "message": "Archive does not exist."}),
                        content_type="application/json; charset=utf-8",
                    )
                ...
                try:
                    if authenticated:
                        ...
                        images = Image.objects.filter(
                            archive=archive_id, position__in=positions, extracted=True
                        ).filter(archive__public=True)
                except Archive.DoesNotExist:
                    return HttpResponseNotFound(
                        json.dumps({"result": "Archive does not exist."}),
                        content_type="application/json; charset=utf-8",
                ...
                image_urls: list[dict[str, typing.Any]] = [
                    {
                        ...
                        "is_horizontal": (
                            image.image_width / image.image_height > 1
                ...
                try:
                    if authenticated:
                        archive = Archive.objects.select_related("gallery").prefetch_related("tags").get(pk=archive_id)
                    else:
                        archive = (
                            Archive.objects.filter(public=True, extracted=True)
                ...
                response = json.dumps(
                    {
                        ...
                        "startImage": (
                            archive.image_set.get(position=position).dump_image(request) if archive.extracted else None
                        ),
                    },
                    # indent=2,
                    sort_keys=True,
            ...
            if "favorites" in data and data["favorites"] and request.user.is_authenticated:
                user_arch_ids = UserArchivePrefs.objects.filter(
                    user=request.user.id, favorite_group=int(data["favorites"])
                ).values_list("archive")
            ...
            archives_list_filtered: "QuerySet[Archive]" = filter_archives(
                request, parameters, display_prms, authenticated, force_private=force_private
            )
            if "extracted" in data:
